...
function convertListToButtons(roomName, occupantList, isPrimary) {
    ...
    for (var easyrtcid in connectList) {
        ...
        label = document.createTextNode("Conectar");
        ...
        label = document.createTextNode("Enviar");
        button.appendChild(label);
        rowGroup.appendChild(button);
    ...
    if (!otherClientDiv.hasChildNodes()) {
        otherClientDiv.innerHTML = "<em>Sala Vazia</em>";
    }
}
...
function loginFailure(errorCode, message) {
    easyrtc.showError(errorCode, "Falha no Login");
}
